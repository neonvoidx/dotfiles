...
|---|---|---|
| jira-mcp + confluence-mcp | Jira (TCT fork from ticketing Artifactory, PAT auth) + Confluence (upstream mcp-atlassian from global-dev, web session). Originally one `atlassian` server; split in oci-dev-starter-pack v0.3.0 because the two halves use different package sources and auth models. | pageId=19068724656 |
...
| aipack report mcp | Local aipack usage telemetry submission and pending queue replay | internal aipack binary |
