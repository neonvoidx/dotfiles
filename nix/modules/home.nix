...
let
  ...
  # Arch Linux-specific directories to exclude from NixOS symlinking
  archSpecificDirs =
    [ "gtk-2.0" "gtk-3.0" "gtk-4.0" "systemd" "paru" "pacman" ];
  ...
  symlinkFiles = sourceDir:
    let
      ...
      findFiles = dir:
        let
          ...
          processEntry = name: type:
            let
              path = "${dir}/${name}";
              # Check if this directory should be excluded (Arch-specific)
              isArchDir = lib.any (archDir:
                lib.hasInfix "/.config/${archDir}" path
                || lib.hasSuffix "/.config/${archDir}" path) archSpecificDirs;
            in if type == "directory" then
              if isArchDir then
                [ ] # Skip Arch-specific directories
              else
      ...
      makeSymlinkEntry = filePath:
        ...
        in lib.nameValuePair relativePath { source = create_symlink filePath; };
    in builtins.listToAttrs (map makeSymlinkEntry files);

  # Determine which directories to symlink based on system
  commonFiles = symlinkFiles "${dotfiles}/common"; # All common dotfiles
  # OS specific
  platformFiles = if pkgs.stdenv.isLinux then
    symlinkFiles "${dotfiles}/linux"
  else if pkgs.stdenv.isDarwin then
    symlinkFiles "${dotfiles}/mac"
  else
    { };
in {
